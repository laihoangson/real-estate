...
def save_incremental_data(new_data_list, file_path):
    ...
    if os.path.exists(file_path) and os.path.getsize(file_path) > 0:
        try:
            ...
            # Ghi đè dòng cũ bằng dòng mới dựa trên Property_ID
...
# Bộ nhớ tracking trạng thái và giá của các listing đã scrape
seen_records = {} 

if os.path.exists(FILE_NAME) and os.path.getsize(FILE_NAME) > 0:
    try:
        df_existing = pd.read_csv(FILE_NAME)
        # Nạp dữ liệu cũ vào memory: { 'ID1': {'Status': 'For Sale', 'Price': '$900k'} }
            ...
            for status_label, mode_url in SEARCH_MODES:
                ...
                for page in range(1, total_pages + 1):
                    ...
                    if script_tag:
                        ...
                        # XỬ LÝ TRỰC TIẾP TỪNG LISTING
                        for pid, item in listings.items():
                            pid_str = str(pid)
                            m = item.get('listingModel', {})
                            
                            # Lấy giá trị giá (price) từ website ngay để kiểm tra
                            raw_price = str(m.get('price', 'N/A'))
                            
                            # Kiểm tra xem có cần bỏ qua listing này không
                            if pid_str in seen_records:
                                old_record = seen_records[pid_str]
                                # Nếu Trạng thái VÀ Giá đều không đổi thì bỏ qua để tiết kiệm thời gian
                                if old_record['Status'] == status_label and old_record['Price'] == raw_price:
                                    continue
                                else:
                                    print(f"      🔄 Phát hiện cập nhật mới cho ID {pid_str} (Đổi status/giá)!")
                            ...
                            # Cập nhật trạng thái và giá mới nhất vào bộ nhớ tạm
                            seen_records[pid_str] = {
                                'Status': status_label,
                                'Price': raw_price
                            }
